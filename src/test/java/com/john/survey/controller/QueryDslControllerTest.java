...
public class QueryDslControllerTest {
    ...
    public void setup() {
        ...
        NpsQuestion npsQuestion1 = npsQuestionRepository.save(new NpsQuestion("aCustomerReference"));
        npsQuestion1.setAnswer(1);
        npsQuestionRepository.save(npsQuestion1);

        NpsQuestion npsQuestion2 = npsQuestionRepository.save(new NpsQuestion("anotherCustomerReference"));
    ...
    public void shouldQueryDslReturnPageableResponseGivenPredicateNull() throws Exception {
        mockMvc.perform(get("/api/v1/query/nps-questions")
    ...
    public void shouldReturnNpsQuestionWithAnswerEqualsTen() throws Exception {
        mockMvc.perform(get("/api/v1/query/nps-questions?answer=10")
                .contentType(contentType))
                .andExpect(status().isOk())
    ...
    public void shouldReturnNpsQuestionWithCustomerReference() throws Exception {
        mockMvc.perform(get("/api/v1/query/nps-questions?customerReference=aCustomerReference")
                .contentType(contentType))
                .andExpect(status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.totalElements").value(1))
                .andExpect(MockMvcResultMatchers.jsonPath("$.content", hasSize(1)))
                .andExpect(MockMvcResultMatchers.jsonPath("$.content[0].customerReference").value("aCustomerReference"));
    }
}